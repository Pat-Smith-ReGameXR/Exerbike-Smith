...
public sealed class GPSEncoder
{
	...
	{
		float m1 = 111132.954f;    // latitude calculation term 1
		float m2 = -559.822f;        // latitude calculation term 2
		...
		//metersPerLon = m1 * (Mathf.Cos((float)lat));
	}
	...
	private Vector2 ConvertUCStoGPS(Vector3 position)
	{
		...
		geoLocation.x = (_LatOrigin + (position.x) / metersPerLat); //Calc current lat
		geoLocation.y = (_LonOrigin + (position.z) / metersPerLon); //Calc current lon
		return geoLocation;
	}
	#endregion
}
